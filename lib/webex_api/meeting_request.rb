module WebexApi
  class MeetingRequest < WebexApi::Request
    ...
    def create_meetings(names,meeting_options)
      body = get_createmeetings_body(names, meeting_options)
      perform_request(body)
    end
    ...
    def get_createmeetings_body(names, meeting_options={})
      body = webex_xml_request(@client.webex_email) do |xml|
        for i in 0..names.length()
          xml.bodyContent('xsi:type' =>'java:com.webex.service.binding.meeting.CreateMeeting'){
            get_meeting_body(xml, names[i], nil, meeting_options[i])
          }
        end
      end
